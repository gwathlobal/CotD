...
(defconstant +custom-scenario-win-time-of-day+ 2)
(defconstant +custom-scenario-win-player-faction+ 3)
...
(defmethod make-output ((win custom-scenario-window))
  ...
  (let ((text-str-num))
    ...
    (sdl:with-rectangle (rect (sdl:rectangle :x 10 :y (+ 10 (sdl:char-height sdl:*default-font*)) :w (- *window-width* 20) :h (- (truncate *window-height* 2) 20)))
      (setf text-str-num (write-text (format nil "Layout: ~A~%Weather: ~A~%Time of day: ~A~%Player faction: ~A"
    ...
    (let ((color-1 sdl:*white*) (color-2 sdl:*white*) (color-3 sdl:*white*) (color-4 sdl:*white*))
      (cond
        ...
        ((= (cur-step win) +custom-scenario-win-time-of-day+) (setf color-3 sdl:*yellow*))
        ((= (cur-step win) +custom-scenario-win-player-faction+) (setf color-4 sdl:*yellow*)))
      
      (sdl:draw-string-solid-* (format nil "1. City layout") 10 (+ 10 (* (sdl:char-height sdl:*default-font*) (+ 2 text-str-num))) :justify :left :color color-1)
      (sdl:draw-string-solid-* (format nil "2. Weather") (* 1 (truncate *window-width* 3)) (+ 10 (* (sdl:char-height sdl:*default-font*) (+ 2 text-str-num))) :justify :center :color color-2)
      (sdl:draw-string-solid-* (format nil "3. Time of day") (* 2 (truncate *window-width* 3)) (+ 10 (* (sdl:char-height sdl:*default-font*) (+ 2 text-str-num))) :justify :center :color color-3)
      (sdl:draw-string-solid-* (format nil "4. Player faction") (- *window-width* 10) (+ 10 (* (sdl:char-height sdl:*default-font*) (+ 2 text-str-num))) :justify :right :color color-4))
    ...
    (let ((cur-str) (color-list nil))
      ...
      (draw-selection-list (menu-items win) cur-str (length (menu-items win)) 20 (+ 10 (* (sdl:char-height sdl:*default-font*) (+ 4 text-str-num))) color-list))
    )
