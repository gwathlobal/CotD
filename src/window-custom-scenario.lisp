...
(defconstant +custom-scenario-win-player-faction+ 5)
...
(defclass custom-scenario-window (window)
  ((cur-step :initform +custom-scenario-win-mission+ :accessor cur-step)
   ...
   (cur-player-faction :initform 0 :initarg :cur-player-faction :accessor cur-player-faction)
   ))
...
(defmethod initialize-instance :after ((win custom-scenario-window) &key)
  ...
  (setf (player-faction-list win) (loop with result = ()
  ...
  (setf (cur-player-faction win) (random (length (player-faction-list win))))
...
(defun readjust-after-faction-change (win)
  (let ((cur-faction-id (nth (cur-player-faction win) (player-faction-list win))))
    (setf (player-faction-list win) (loop with result = ()
                                          for (faction-id faction-present) in (cur-faction-list win)
                                          for faction-obj = (get-faction-type-by-id faction-id)
                                          when (or (= faction-present +mission-faction-attacker+)
    ...
    (if (position cur-faction-id (player-faction-list win))
      (progn
        (setf (cur-player-faction win) (position cur-faction-id (player-faction-list win))))
      (progn
        (setf (cur-player-faction win) (random (length (player-faction-list win))))))
    ))
  ...
  (let ((text-str-num))
    ...
    (sdl:with-rectangle (rect (sdl:rectangle :x 10 :y (+ 10 (sdl:char-height sdl:*default-font*)) :w (- *window-width* 20) :h (- (truncate *window-height* 2) 20)))
      (setf text-str-num (write-text (format nil "Mission: ~A~%Layout: ~A~%Weather: ~A~%Time of day: ~A~%Factions: ~A~%Player faction: ~A"
                                             ...
                                             (sf-name (get-scenario-feature-by-id (nth (cur-player-faction win) (player-faction-list win))))
                                             )
                                     rect)))
    ...
    (:key-down-event (:key key :mod mod :unicode unicode)
                     ...
                     (cond
                       ...
                       ((= (cur-step win) +custom-scenario-win-factions+) (progn (setf (cur-faction win) (run-selection-list key mod unicode (cur-faction win)))
                                                                                 ...
                                                                                 (readjust-after-faction-change win)))
                       ((= (cur-step win) +custom-scenario-win-player-faction+) (progn (setf (cur-player-faction win) (run-selection-list key mod unicode (cur-player-faction win)))
                                                                                       (setf (cur-player-faction win) (adjust-selection-list (cur-player-faction win) (length (player-faction-list win))))
                                                                                       (setf (cur-sel win) (cur-player-faction win)))))
