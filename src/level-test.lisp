...
(defun test-level-place-mobs (world mob-template-list)
  ...
  (let ((soldier (make-instance 'mob :mob-type +mob-type-angel+ :x (+ (x *player*) 7) :y (- (y *player*) 1)))
        (demon (make-instance 'mob :mob-type +mob-type-gargantaur+ :x (+ (x *player*) 4) :y (+ (y *player*) 0)))
        )
    ;(setf (cur-hp demon) 1)
