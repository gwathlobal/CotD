...
(defun test-level-place-mobs (world mob-template-list)
  ...
  (setf *player* (make-instance 'player :mob-type +mob-type-scout+ :x 43 :y 15))
  (add-mob-to-level-list (level world) *player*)
  (let ((soldier (make-instance 'mob :mob-type +mob-type-archangel+ :x (+ (x *player*) 3) :y (+ (y *player*) 3)))
        (demon (make-instance 'mob :mob-type +mob-type-demon+ :x (+ (x *player*) 1) :y (+ (y *player*) 0))))
    ;(setf (cur-hp demon) 1)
