...
(defun generate-test-world-map (world-map world-time)
  ...
  (setf (aref (cells world-map) 2 1) (make-instance 'world-sector :wtype +world-sector-normal-port+
                                                                  :feats (list (list +lm-feat-river+ nil))))
  (setf (aref (cells world-map) 3 1) (make-instance 'world-sector :wtype +world-sector-normal-port+))
  (setf (aref (cells world-map) 4 1) (make-instance 'world-sector :wtype +world-sector-normal-port+))
  ...
  (setf (aref (cells world-map) 2 2) (make-instance 'world-sector :wtype +world-sector-corrupted-residential+
                                                                  :feats (list (list +lm-feat-river+ nil))
                                                                  :controlled-by +lm-controlled-by-demons+))
  (setf (aref (cells world-map) 3 2) (make-instance 'world-sector :wtype +world-sector-normal-lake+
                                                                  :feats (list (list +lm-feat-river+ nil))
  ...
  (setf (aref (cells world-map) 1 3) (make-instance 'world-sector :wtype +world-sector-corrupted-residential+
                                                                  :controlled-by +lm-controlled-by-demons+))
  (setf (aref (cells world-map) 2 3) (make-instance 'world-sector :wtype +world-sector-normal-residential+
                                                                  :feats (list (list +lm-feat-river+ nil) (list +lm-feat-lair+) (list +lm-feat-church+))
                                                                  :items (list +lm-item-holy-relic+)))
  (setf (aref (cells world-map) 3 3) (make-instance 'world-sector :wtype +world-sector-corrupted-residential+
                                                                  :controlled-by +lm-controlled-by-demons+))
  ...
  (setf (aref (cells world-map) 2 4) (make-instance 'world-sector :wtype +world-sector-corrupted-forest+
                                                                  :feats (list (list +lm-feat-river+ nil))
                                                                  :controlled-by +lm-controlled-by-demons+))
...
(defun generate-feats-on-world-map (world-map)
  (loop for y from 0 below *max-y-world-map* do
    (loop for x from 0 below *max-x-world-map*
          for river-feat = (find +lm-feat-river+ (feats (aref (cells world-map) x y)) :key #'(lambda (a) (first a)))
          for sea-feat = (find +lm-feat-sea+ (feats (aref (cells world-map) x y)) :key #'(lambda (a) (first a)))
          for barricade-feat = (find +lm-feat-barricade+ (feats (aref (cells world-map) x y)) :key #'(lambda (a) (first a)))
          ...
          until (and (not (mission world-sector))
             ...
             (let ((mission-type-id (nth (random (length avail-missions)) avail-missions))
               ...
               (setf (mission world-sector)
                     (make-instance 'mission :mission-type-id mission-type-id
                                             :x rx :y ry
